...
<body bgcolor="darkcyan">
  ...
  <form method="post" onsubmit="return confirmInsert();">
    ...
    <input type="number" id="log_id" name="log_id" required><br><br>
    ...
    <input type="text" id="message_content" name="message_content" required><br><br>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO chatlogs (log_id, message_id, sender_name, meeting_id, message_content, timestamp) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iissss", $log_id, $message_id, $sender_name, $meeting_id, $message_content, $timestamp);

    if ($stmt->execute()) {
      echo "New record has been added successfully.<br><br><a href='chatlogs.php'>Back to Form</a>";
    } else {
  ...
  <section>
    ...
    <table>
      ...
      <?php
      // Fetch and display chat log records
      $sql = "SELECT * FROM chatlogs";
      ...
      if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          echo "<tr>
                  ...
                  <td>{$row['timestamp']}</td>
                  <td><a style='padding:4px' href='delete chatlogs.php?log_id={$row['log_id']}'>Delete</a></td>
                  <td><a style='padding:4px' href='update chatlogs.php?log_id={$row['log_id']}'>Update</a></td>
                </tr>";
        }
      } else {
